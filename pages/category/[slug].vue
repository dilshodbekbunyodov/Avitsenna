...
slugId = await useFetchSlugId(route.params.slug) as { id: string };
const {seo, breadcrumbs} = await useFetchSlugIdSeo(route.params.slug)

const rubric = {lang: 'uz', "filter[rubric_id]": slugId.id, per_page: 8, page: currentPage.value};

await useFetchDetailArticles(rubric)
    .then((res: any) => {
      articles.value = res
    })
